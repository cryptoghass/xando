...
class TxViewDelegate2 : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate2() : QAbstractItemDelegate(), unit(BitcoinUnits::XDO)
    {
    }
...
void Coinmix::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("XDO")
    //updateDisplayUnit();
}
...
void Coinmix::updateDarksendProgress()
{
    ...
    QString strAnonymizeXDOAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeXDOAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeXDOAmount = strAnonymizeXDOAmount.remove(strAnonymizeXDOAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeXDOAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeXDOAmount * COIN) nMaxToAnonymize = nAnonymizeXDOAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeXDOAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeXDOAmount));
        strAnonymizeXDOAmount = strAnonymizeXDOAmount.remove(strAnonymizeXDOAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeXDOAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void Coinmix::DarksendAuto()
{
    DarKsendPool.DoAutomaticDenominating();
}

void Coinmix::DarksendReset()
{
    DarKsendPool.Reset();
