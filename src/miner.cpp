...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("XDOMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("XDOMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("XDOMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("xando-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("XDOMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running XDOMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadXDOMiner() exception");
    } catch (...) {
        LogPrintf("ThreadXDOMiner() exception");
    }

    LogPrintf("ThreadXDOMiner exiting\n");
}
